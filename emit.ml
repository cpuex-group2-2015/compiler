...
and g' oc = function (* 各命令のアセンブリ生成 *)
  ...
  | (NonTail(x), FMul(y, z)) ->
     ...
     address := !address + step
  | (NonTail(x), FDiv(y, z)) ->
     Printf.fprintf oc "\tfinv\tftmp, %s\n" (reg z);
     Printf.fprintf oc "\tfmul\t%s, %s, ftmp\n" (reg x) (reg y);
     file := !file ^ Printf.sprintf "1111111111100000%s00000100100\n" (reg_to_binary (reg z));
     file := !file ^ Printf.sprintf "111111%s%s1111100000110010\n" (reg_to_binary (reg x)) (reg_to_binary (reg y));
     address := !address + step * 2
  | (NonTail(x), Lfd(y, V(z))) ->
...
let f oc bc zc p =
  ...
  file := !file ^ "01001100000000000000000000000000\n"; (* blr *)
  file := !file ^ "00111000010000000000000000000000\n"; (* li # sqrt_sub *)
  file := !file ^ "00111100010000000011111100000000\n"; (* lis *)
  file := !file ^ "01010100001000100000000000000000\n"; (* mfgtf *)
  file := !file ^ "11111100001000000000100000110010\n"; (* fmul *)
  file := !file ^ "00111100010000000011111111000000\n"; (* lis *)
  file := !file ^ "01010100010000100000000000000000\n"; (* mfgtf *)
  file := !file ^ "01011000010000000000000000000000\n"; (* mfftg *)
  file := !file ^ "00111000101000000000000000000001\n"; (* li *)
  file := !file ^ "01111100010000100010110000110000\n"; (* sr *)
  file := !file ^ "00111100101000000101111100110111\n"; (* lis *)
  file := !file ^ "00111000101001010101100111011111\n"; (* addi *)
  file := !file ^ "01111100010000100000000011010000\n"; (* neg *)
  file := !file ^ "01111100010001010001001000010100\n"; (* add *)
  file := !file ^ "01010100000000100000000000000000\n"; (* mfgtf *)
  file := !file ^ "11111100011000000000000000110010\n"; (* fmul *)
  file := !file ^ "11111100011000010001100000110010\n"; (* fmul *)
  file := !file ^ "11111100011000000001100001010000\n"; (* fneg *)
  file := !file ^ "11111100011000100001100000101010\n"; (* fadd *)
